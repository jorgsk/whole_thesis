...
TSS \cite{ross_third_1993,lilian_m_promoter_2002}. RNAP cannot bind strongly to
...
promoters when in complex with RNAP \cite{paget_$sigma^70$_2003}, making
...
as well as the distance in nucleotides between these elements. A third sequence
that can affect promoter-RNAP-$\sigma^{70}$ binding is an AT rich element
upstream -35 called, simply, the upstream element \cite{ross_third_1993,
...
will be, which again may lead to higher expression of the downstream gene.
However, we will shortly see that the initial transcribed sequence, defined as
the region from +1 to +20 realtive to the TSS (Figure \ref{fig:promoter}), may
also influence the expression from a promoter.
...
% TODO: description of Hsu's experimenta
To quantify abortive initiation, one must obtain a measure of the molar
abundance of each abortive RNA. Additionally, abortive RNAs of a given length
must be quantified separately from any possible cleavage products. A method
that fulfills these two requirements is radioactive labeling of nucleotide
\gamma-phosphates \cite{hsu_monitoring_2009}. Each abortive RNA will only
contain one \gamma-phosphate, namely the one at the 5\ppp end. All other
\gamma-phosphates are cleaved off during transcription and released in
phyrophosphate (PPi). By separating abortive RNAs on a single-nucleotide
resolution gel, one can then use a phosphoimager to quantify the amount of each
abortive product \cite{hsu_monitoring_2009}.

By quantifying the amount of abortive RNAs of different lengths, one can infer
at which nucleotide positions abortive initiation is more or less prominent
\cite{hsu_<i>vitro</i>_2003}. However, when performing this kind of analysis ->
abortive init -> fewer RNAP reach later stages, so must normalize for this ->
AP.

When using this approach, one finds that the amount of abortive RNAs of different lenghts 
